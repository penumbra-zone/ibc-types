...
impl ClientId {
    ...
    /// ```
    // XXX changed from ClientType to AsRef<str> to avoid circular dependency
    pub fn new(client_type: impl AsRef<str>, counter: u64) -> Result<Self, ValidationError> {
        let prefix = client_type.as_ref();
