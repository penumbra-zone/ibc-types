...
use tendermint::abci::{self, Event, EventAttributeIndexExt, TypedEvent};
...
impl CreateClient {
    // XXX: add to `tendermint-rs` `TypedEvent`?
    pub const EVENT_KIND: &str = "create_client";
}
...
impl From<CreateClient> for Event {
    fn from(c: CreateClient) -> Self {
        Event::new(
            CreateClient::EVENT_KIND,
            [
                (CLIENT_ID_ATTRIBUTE_KEY, c.client_id.to_string()).index(),
                (CLIENT_TYPE_ATTRIBUTE_KEY, c.client_type.to_string()).index(),
                (
                    CONSENSUS_HEIGHT_ATTRIBUTE_KEY,
                    c.consensus_height.to_string(),
                )
                    .index(),
            ],
        )
